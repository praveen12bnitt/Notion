 ...
 * holder.
 */
package edu.mayo.qia.pacs.http;
 ...
 * @author Grizzly Team
 */
public class LocalStaticHttpHandler extends StaticHttpHandlerBase {
  private static final Logger LOGGER = Grizzly.logger(LocalStaticHttpHandler.class);
  ...
  /**
   * Create <tt>HttpHandler</tt>, which will handle requests to the static
   * resources resolved by the given class loader.
   * 
   * @param {@link ClassLoader} to be used to resolve the resources
   * @param docRoots
   *          the doc roots (path prefixes), which will be used to find
   *          resources. Effectively each docRoot will be prepended to a
   *          resource path before passing it to
   *          {@link ClassLoader#getResource(java.lang.String)}. If no
   *          <tt>docRoots</tt> are set - the resources will be searched
   *          starting from {@link ClassLoader}'s root.
   * @throws IllegalArgumentException
   *           if one of the docRoots doesn't end with slash ('/')
   */
  public LocalStaticHttpHandler(final ClassLoader classLoader, final String... docRoots) {
    if (classLoader == null) {
   ...
   * 
   * @param docRoot
   * @return <tt>true</tt> if this docroot hasn't been registered before, or
   ...
   * 
   * @param docRoot
   * @return <tt>true</tt> if this docroot was found and removed from the list,
   *         or or <tt>false</tt> if this docroot was not found in the list.
   */
  ...
  /**
   * Returns the {@link ClassLoader} used to resolve the requested HTTP
   * resources.
   */
  ...
  protected boolean handle(String resourcePath, final Request request, final Response response) throws Exception {
    ...
    if (url != null) {
      if ("file".equals(url.getProtocol())) {
        ...
        if ("jar".equals(url.getProtocol())) {
          ...
          // we can't rely on jarEntry.isDirectory() because of
          // http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=6233323
          InputStream is = jarFile.getInputStream(jarEntry);
          jarResource = jarEntry;
          if (jarEntry.isDirectory() || is == null || is.available() == 0) { // it's
                                                                             // probably
                                                                             // a
                                                                             // folder
            final JarEntry welcomeJarEntry = jarFile.getJarEntry(jarEntry.getName() + "index.html");
            if (welcomeJarEntry != null) {
              is = jarFile.getInputStream(welcomeJarEntry);
              jarResource = welcomeJarEntry;
            }
          }
    ...
    if (fileResource != null) {
      ...
      pickupContentType(response, jarResource.getName());
