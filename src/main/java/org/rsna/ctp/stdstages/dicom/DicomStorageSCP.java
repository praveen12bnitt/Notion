...
public class DicomStorageSCP extends DcmServiceBase {
  ...
  protected void doCStore(ActiveAssociation assoc, Dimse rq, Command rspCmd) throws IOException {
    ...
    try {
      ...
      boolean accept = ipWhiteList.contains(connectionIP) && !ipBlackList.contains(connectionIP) && calledAETWhiteList.contains(calledAET) && !calledAETBlackList.contains(calledAET) && callingAETWhiteList.contains(callingAET)
          && !callingAETBlackList.contains(callingAET);

      // Log the connection if logging is enabled
      if (logAllConnections || (!accept && logRejectedConnections)) {
        logger.warn(dicomImportService.getName() + (accept ? " accepted " : " rejected ") + currentUID + " from " + connectionIP + "(" + calledAET + ":" + callingAET + ")");
      }
      ...
      if (!accept) {
        ...
        // *********************************************************************************************
        // doCStore may be called from multiple threads in the DICOM library
        // simultaneously.
        // This section must be synchronized because the LinkedList class is not
        // thread-safe.
  ...
  private void storeToDir(InputStream in, FileMetaInfo fmi, String calledAET, String callingAET, String connectionIP, long time) throws IOException {
  ...
  // Skip an object completely. This is called if we are suppressing recent
  // duplicates.
  ...
  class Handler extends Thread {
    ...
    private File setAET() {
      if ((calledAETTag == 0) && (callingAETTag == 0) && (connectionIPTag == 0) && (timeTag == 0))
        return file;
      DicomObject dob = null;
      try {
        dob = new DicomObject(file, true);
        if (calledAETTag != 0)
          dob.setElementValue(calledAETTag, calledAET);
        if (callingAETTag != 0)
          dob.setElementValue(callingAETTag, callingAET);
        if (connectionIPTag != 0)
          dob.setElementValue(connectionIPTag, connectionIP);
        if (timeTag != 0)
          dob.setElementValue(timeTag, Long.toString(time));
