...
/**
 * A {@link Reservoir} implementation backed by a sliding window that stores
 * only the measurements made
...
public class SlidingTimeWindowReservoir implements Reservoir {
  ...
  /**
   * Creates a new {@link SlidingTimeWindowReservoir} with the given window of
   * time.
   *
   * @param window
   *        the window of time
   * @param windowUnit
   *        the unit of {@code window}
   */
  ...
  /**
   * Creates a new {@link SlidingTimeWindowReservoir} with the given clock and
   * window of time.
   *
   * @param window
   *        the window of time
  ...
  private long getTick() {
    for (;;) {
      ...
      // ensure the tick is strictly incrementing even if there are duplicate
      // ticks
